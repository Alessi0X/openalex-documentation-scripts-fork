...
if __name__ == "__main__":
    # Define the list of flattening functions
    ...
    # Create a process pool and run functions in parallel with progress bar
    with mp.Pool(processes=min(len(flattening_functions), mp.cpu_count())) as pool:
        # Use tqdm to show progress as processes complete
        results = list(
            tqdm(
                pool.imap(lambda x: x(), flattening_functions),
                total=len(flattening_functions),
                desc="Flattening entities",
            )
        )

    print(f"All {len(flattening_functions)} flattening processes completed.")
